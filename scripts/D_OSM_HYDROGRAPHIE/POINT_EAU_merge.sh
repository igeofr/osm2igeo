...
then
  ...
  for i in $(ls "./data_temp/$PAYS/$OUT_EPSG/D_OSM_HYDROGRAPHIE/POINT_EAU/"*.gpkg)
  do
    echo "merge $i"
    $LINK_OGR -progress -f 'GPKG' -update -append --debug ON -lco SPATIAL_INDEX=YES $file $i $(basename "${i%.*}") -nln POINT_EAU
  done
fi
if [ "$FORMAT_SIG" = "SQL" ]
then
  file="./data_temp/$PAYS/$OUT_EPSG/D_OSM_HYDROGRAPHIE/POINT_EAU.sql"
  ( cat "./scripts_V3/D_OSM_HYDROGRAPHIE/POINT_EAU_create.sql" ; cat "./data_temp/$PAYS/$OUT_EPSG/D_OSM_HYDROGRAPHIE/POINT_EAU/"*.sql) > $file
fi
