...
echo "Debut : D_OSM_HYDROGRAPHIE > SURFACE_EAU_fleuve_riviere.shp"
$LINK_OGR -progress -s_srs EPSG:4326 -t_srs EPSG:$OUT_EPSG -f 'ESRI Shapefile' 'data_temp/'$PAYS/$OUT_EPSG'/D_OSM_HYDROGRAPHIE/SURFACE_EAU/SURFACE_EAU_fleuve_riviere.shp' -dialect SQLITE -sql "SELECT * FROM (
------------------------------------------------------------------------------------------------------------
------------------------------------ Fleuve, rivière -----------------------------------------------------
------------------------------------------------------------------------------------------------------------
SELECT
...
echo "Fin : D_OSM_HYDROGRAPHIE > SURFACE_EAU_fleuve_riviere.shp"
