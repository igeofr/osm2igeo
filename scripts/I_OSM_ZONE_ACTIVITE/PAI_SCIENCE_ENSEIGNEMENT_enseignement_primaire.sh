...
var_sub_group=PAI_SCIENCE_ENSEIGNEMENT
var_file=PAI_SCIENCE_ENSEIGNEMENT_enseignement_primaire
#------------------------------------------------------------------------------------------------------------
...
-----------------------------------------
FROM multipolygons WHERE amenity='school' AND (school_FR='maternelle' OR school_FR='élémentaire' OR school_FR='primaire') AND name IS NOT NULL AND ST_IsValid(ST_Buffer(GEOMETRY,0))
-----------------------------------------
UNION
-----------------------------------------
SELECT
...
SUBSTR(osm_timestamp, 1, 10) AS "DATE_MAJ"
-----------------------------------------
FROM points WHERE amenity='school' AND (school_FR='maternelle' OR school_FR='élémentaire' OR school_FR='primaire') AND name IS NOT NULL AND ST_IsValid(GEOMETRY)"
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
if [ "$FORMAT_SIG" = "SHP" ]
