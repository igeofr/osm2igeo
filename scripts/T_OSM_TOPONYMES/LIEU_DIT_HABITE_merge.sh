...
then
  ...
  for i in $(ls "./data_temp/$PAYS/$OUT_EPSG/T_OSM_TOPONYMES/LIEU_DIT_HABITE/"*.gpkg)
  do
    echo "merge $i"
    $LINK_OGR -progress -f 'GPKG' -update -append --debug ON -lco SPATIAL_INDEX=YES $file $i $(basename "${i%.*}") -nln LIEU_DIT_HABITE
  done
fi
if [ "$FORMAT_SIG" = "SQL" ]
then
  file="./data_temp/$PAYS/$OUT_EPSG/T_OSM_TOPONYMES/LIEU_DIT_HABITE.sql"
  ( cat "./scripts_V3/T_OSM_TOPONYMES/LIEU_DIT_HABITE_create.sql" ; cat "./data_temp/$PAYS/$OUT_EPSG/T_OSM_TOPONYMES/LIEU_DIT_HABITE/"*.sql) > $file
fi
