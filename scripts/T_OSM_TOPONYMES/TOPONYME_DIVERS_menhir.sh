...
echo "Debut : T_OSM_TOPONYMES > TOPONYME_DIVERS_menhir.shp"
$LINK_OGR -progress -s_srs EPSG:4326 -t_srs EPSG:$OUT_EPSG -f 'ESRI Shapefile' 'data_temp/'$PAYS/$OUT_EPSG'/T_OSM_TOPONYMES/TOPONYME_DIVERS/TOPONYME_DIVERS_menhir.shp' -dialect SQLITE -sql "SELECT * FROM (
------------------------------------------------------------------------------------------------------------
------------------------------------ Menhir ----------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
SELECT
...
echo "Fin : T_OSM_TOPONYMES > TOPONYME_DIVERS_menhir.shp"
